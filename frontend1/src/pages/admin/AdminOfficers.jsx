...
export default function AdminOfficers() {
  ...
  const { data: locations } = useQuery({
    queryKey: ['admin-officer-locations'],
    queryFn: () => api.get('/admin/officers/locations').then(r => r.data.data),
  });

  const { data, isLoading } = useQuery({
    queryKey: ['admin-officers', page, search, rankId, thana, zone, availability, status],
    queryFn: () => api.get('/admin/officers', {
      params: { page, limit: 15, search, rankId, thana, zone, availability, status },
    }).then(r => r.data.data),
  });
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold text-gray-900 dark:text-white">अधिकारी — Officers</h1>
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Every officer under you — filter and click a row for full details.</p>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 dark:bg-gray-800/50">
              <tr>
                {['Officer', 'Badge', 'Rank', 'Thana', 'Zone', 'Availability', 'Status'].map(h => (
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
              {isLoading ? (
                <tr><td colSpan={7} className="text-center py-10"><div className="w-6 h-6 border-2 border-primary-200 border-t-primary-600 rounded-full animate-spin mx-auto" /></td></tr>
              ) : data?.data?.length === 0 ? (
      ...
      <OfficerDetailModal isOpen={!!viewTarget} onClose={() => setViewTarget(null)} officer={viewTarget} />
    </div>
  );
}